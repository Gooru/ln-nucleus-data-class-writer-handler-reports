  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    try {
      ...
      if (resourceEvent.getCollectionId() != null) {
        LazyList<AJEntityCollectionPerformance> duplicateRow = AJEntityCollectionPerformance
            .findBySQL(AJEntityCollectionPerformance.CHECK_DUPLICATE_COLLECTION_EVENT,
                resourceEvent.getUser(),
        ...
        if (duplicateRow != null && !duplicateRow.isEmpty()) {
          ...
          duplicateRow.forEach(dup -> {
            ...
            long ts = (Long.valueOf(dup.get("timespent").toString()) + resourceEvent
                .getTimeSpent());
            Base.exec(AJEntityCollectionPerformance.UPDATE_COLLECTION_TIMESPENT, ts,
                new Timestamp(resourceEvent.getActivityTime()), id);
          });
        } else {
          LOGGER.warn(
              "RDA-RSE:: Update Skipped! coll start event either missing or some issue. RDA-EVENT: {}",
              context.request());
        }
      }
    } catch (Exception e) {
