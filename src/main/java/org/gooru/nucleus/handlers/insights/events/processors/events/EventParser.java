	...
	private EventParser parse() {
		try {			
			...
			if(context.containsKey(EventConstants.COLLECTION_TYPE) && context.getString(EventConstants.COLLECTION_TYPE).equals(EventConstants.COLLECTION)){
				...
				this.collectionType = context.containsKey(EventConstants.COLLECTION_TYPE) ? context.getString(EventConstants.COLLECTION_TYPE) : null;
			}
	...
	public void processCollectionPlayEvents(){
		...
		if (this.eventType.equals(EventConstants.START) && (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION) || 
			...
			if(this.collectionType.equalsIgnoreCase(EventConstants.EXTERNAL_ASSESSMENT) || 
					this.collectionType.equalsIgnoreCase(EventConstants.EXTERNAL_COLLECTION)){
				this.timespent = metrics.containsKey(EventConstants.TIMESPENT) ? metrics.getLong(EventConstants.TIMESPENT) :  0L;
				this.maxScore = metrics.containsKey(EventConstants.MAX_SCORE) ? metrics.getDouble(EventConstants.MAX_SCORE) :  0.0;	
			} else {
