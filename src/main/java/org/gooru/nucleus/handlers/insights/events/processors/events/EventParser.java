...
public final class EventParser {
  ...
  private EventParser parse() {
    try {
      ...
      this.timespent = 0;
      this.apiKey =
          session.containsKey(EventConstants.API_KEY) ? session.getString(EventConstants.API_KEY)
              : EventConstants.NA;
      this.contentGooruId = context.containsKey(EventConstants.CONTENT_GOORU_OID) ? context
          .getString(EventConstants.CONTENT_GOORU_OID) : EventConstants.NA;

      this.gooruUUID = user.getString(EventConstants.GOORUID);
      this.tenantId = context.containsKey(EventConstants.TENANT_ID) ? context
          .getString(EventConstants.TENANT_ID) : null;
      this.lessonGooruId = context.containsKey(EventConstants.LESSON_GOORU_OID) ? context
          .getString(EventConstants.LESSON_GOORU_OID) : null;
      this.unitGooruId = context.containsKey(EventConstants.UNIT_GOORU_OID) ? context
          .getString(EventConstants.UNIT_GOORU_OID) : null;
      this.courseGooruId = context.containsKey(EventConstants.COURSE_GOORU_OID) ? context
          .getString(EventConstants.COURSE_GOORU_OID) : null;
      this.classGooruId = context.containsKey(EventConstants.CLASS_GOORU_OID) ? context
          .getString(EventConstants.CLASS_GOORU_OID) : null;
      this.parentGooruId = context.containsKey(EventConstants.PARENT_GOORU_OID) ? context
          .getString(EventConstants.PARENT_GOORU_OID) : EventConstants.NA;
      this.parentEventId = context.containsKey(EventConstants.PARENT_EVENT_ID) ? context
          .getString(EventConstants.PARENT_EVENT_ID) : EventConstants.NA;
      if (context.containsKey(EventConstants.COLLECTION_TYPE) && context
          .getString(EventConstants.COLLECTION_TYPE).equals(EventConstants.COLLECTION)) {
        this.collectionType = EventConstants.COLLECTION;
      } else if (context.containsKey(EventConstants.COLLECTION_TYPE) && context
          .getString(EventConstants.COLLECTION_TYPE).equals(EventConstants.ASSESSMENT)) {
        this.collectionType = EventConstants.ASSESSMENT;
      } else {
        this.collectionType = context.containsKey(EventConstants.COLLECTION_TYPE) ? context
            .getString(EventConstants.COLLECTION_TYPE) : null;
      }
      this.resourceType = context.containsKey(EventConstants.RESOURCE_TYPE) ? context
          .getString(EventConstants.RESOURCE_TYPE) : EventConstants.NA;
      this.eventType =
          context.containsKey(EventConstants.TYPE) ? context.getString(EventConstants.TYPE)
              : EventConstants.NA;
      this.appId =
          context.containsKey(EventConstants.APP_ID) ? context.getString(EventConstants.APP_ID)
              : null;
      this.partnerId = context.containsKey(EventConstants.PARTNER_ID) ? context
          .getString(EventConstants.PARTNER_ID) : null;
      this.collectionSubType = context.containsKey(EventConstants.COLLECTION_SUB_TYPE) ? context
          .getString(EventConstants.COLLECTION_SUB_TYPE) : null;
      this.pathId =
          context.containsKey(EventConstants.PATH_ID) ? context.getLong(EventConstants.PATH_ID)
              : 0L;
      this.pathType = context.containsKey(EventConstants.PATH_TYPE) ? context
          .getString(EventConstants.PATH_TYPE) : null;
      this.sessionId = session.containsKey(EventConstants.SESSION_ID) ? session
          .getString(EventConstants.SESSION_ID) : EventConstants.NA;
      this.questionType = payLoadObject.containsKey(EventConstants.QUESTION_TYPE) ? payLoadObject
          .getString(EventConstants.QUESTION_TYPE) : EventConstants.NA;
      if (payLoadObject.containsKey(EventConstants.ANSWER_OBECT) && payLoadObject
          .getValue(EventConstants.ANSWER_OBECT) instanceof JsonArray) {
        this.answerObject = payLoadObject.getJsonArray(EventConstants.ANSWER_OBECT);
      } else {
        this.answerObject = new JsonArray();
      }
      this.sourceId =
          payLoadObject.containsKey("sourceId") ? payLoadObject.getString("sourceId") : null;
      this.reportsContext =
          payLoadObject.containsKey(EventConstants.REPORTS_CONTEXT) ? payLoadObject
              .getString(EventConstants.REPORTS_CONTEXT) : EventConstants.PERFORMANCE;
      this.gradeType = payLoadObject.containsKey(EventConstants.GRADING_TYPE) ? payLoadObject
          .getString(EventConstants.GRADING_TYPE) : EventConstants.SYSTEM;
      this.gradeStatus = payLoadObject.containsKey(EventConstants.GRADE_STATUS) ? payLoadObject
          .getString(EventConstants.GRADE_STATUS) : EventConstants.NA;
      this.teacherId = payLoadObject.containsKey(EventConstants.TEACHER_ID) ? payLoadObject
          .getString(EventConstants.TEACHER_ID) : EventConstants.NA;
      this.contentFormat = payLoadObject.containsKey(EventConstants.CONTENT_FORMAT) ? payLoadObject
          .getString(EventConstants.CONTENT_FORMAT) : EventConstants.NA;
      this.questionCount = context.containsKey(EventConstants.QUESTION_COUNT) ? context
          .getInteger(EventConstants.QUESTION_COUNT) : 0;
      if (payLoadObject.containsKey(EventConstants.TAXONOMYIDS) && payLoadObject
          .getValue(EventConstants.TAXONOMYIDS) instanceof JsonObject) {
      ...
      this.setStudent(
          (!payLoadObject.containsKey(EventConstants.IS_STUDENT)) || payLoadObject
              .getBoolean(EventConstants.IS_STUDENT));
      this.eventTime = this.event.getLong(EventConstants.END_TIME);
      this.collectionItemId = EventConstants.NA;
      this.score =
          metrics.containsKey(EventConstants.SCORE) ? metrics.getDouble(EventConstants.SCORE) : 0;
      ...
      this.contentSource = context.containsKey(EventConstants.CONTENT_SOURCE) ? context
          .getString(EventConstants.CONTENT_SOURCE) : null;

      this.timezone =
          this.event.containsKey(EventConstants.TIMEZONE) ? event.getString(EventConstants.TIMEZONE)
              : null;

      this.contextCollectionId =
          this.event.containsKey(EventConstants.CONTEXT_COLLECTION_ID) ? event
              .getString(EventConstants.CONTEXT_COLLECTION_ID) : null;

      this.contextCollectionType =
          this.event.containsKey(EventConstants.CONTEXT_COLLECTION_TYPE) ? event
              .getString(EventConstants.CONTEXT_COLLECTION_TYPE) : null;

      //this.sourceId = (context.containsKey("source") ? context.getString("source") : null);
      this.accessToken = (this.session.containsKey("sessionToken") ? this.session
  ...
  public void processCollectionPlayEvents() {
    ...
    if (this.eventType.equals(EventConstants.START) && (
      ...
      if (this.collectionType.equalsIgnoreCase(EventConstants.EXTERNAL_ASSESSMENT) ||
          this.collectionType.equalsIgnoreCase(EventConstants.EXTERNAL_COLLECTION)) {
        this.timespent = metrics.containsKey(EventConstants.TIMESPENT) ? metrics
            .getLong(EventConstants.TIMESPENT) : 0L;
        this.maxScore = metrics.containsKey(EventConstants.MAX_SCORE) ? metrics
            .getDouble(EventConstants.MAX_SCORE) : 0.0;
      } else {
