...
public class OfflineStudentReportingVerticle extends AbstractVerticle {
  ...
  private static final Logger LOGGER = LoggerFactory
      .getLogger(OfflineStudentReportingVerticle.class);
  ...
  public void start(Future<Void> voidFuture) throws Exception {
    ...
    eb.consumer(MessagebusEndpoints.MBEP_ANALYTICS_OFFLINE_REPORT, message -> {
      LOGGER
          .debug("Received message at the Offline Student Reporting Verticle: {}", message.body());
      vertx.executeBlocking(future -> {
        MessageResponse result = ProcessorBuilder.buildOfflineStudentReportingProcessor(message)
            .process();
      ...
      if (result.succeeded()) {
        ...
        LOGGER.error(
            "Error registering the Offline Student Reporting Verticle. Halting the Class Reporting machinery");
