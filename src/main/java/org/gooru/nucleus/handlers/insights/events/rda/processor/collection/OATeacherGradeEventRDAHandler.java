 ...
 * @author renuka@gooru
 */
public class OATeacherGradeEventRDAHandler implements DBHandler {

  private static final Logger LOGGER = LoggerFactory
      .getLogger(OATeacherGradeEventRDAHandler.class);
  ...
  public OATeacherGradeEventRDAHandler(RDAProcessorContext context) {
      ...
      if (collectionReport.isValid()) {
        if (duplicateRow == null || duplicateRow.isEmpty()) {
          if (collectionReport.insert()) {
            LOGGER.info("Record inserted successfully in collperf table of Reports DB");
          } else {
            LOGGER.error(
                "Error while inserting event into collperf table of Reports DB: " + context.request().toString());
          }
        } else {
