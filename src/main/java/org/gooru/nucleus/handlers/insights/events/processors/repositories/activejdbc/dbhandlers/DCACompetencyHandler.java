...
public class DCACompetencyHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    dcacompetencyReport
        .set(AJEntityDCACompetencyReport.CREATED_AT, new Timestamp(event.getStartTime()));
    dcacompetencyReport
        .set(AJEntityDCACompetencyReport.UPDATED_AT, new Timestamp(event.getEndTime()));

    Object dcaReportId = Base
        .firstCell(AJEntityDailyClassActivity.SELECT_DCA_REPORT_ID, event.getParentGooruId(),
    ...
    List<AJEntityDCACompetencyReport> reports = AJEntityDCACompetencyReport
        .where(AJEntityDCACompetencyReport.SELECT_ROWS_BY_SESSION_ID_AND_RESOURCE,
            event.getSessionId(), event.getContentGooruId(), event.getEventType());
    if (reports.isEmpty()) {
      ...
      if (!event.getTaxonomyIds().isEmpty()) {
        for (String internalTaxonomyCode : event.getTaxonomyIds().fieldNames()) {
          ...
          taxCompetency.set(AJEntityDCACompetencyReport.TAX_SUBJECT_ID,
              taxObject.get(MessageConstants.SUBJECT));
          taxCompetency.set(AJEntityDCACompetencyReport.TAX_COURSE_ID,
              taxObject.get(MessageConstants.COURSE));
          taxCompetency.set(AJEntityDCACompetencyReport.TAX_DOMAIN_ID,
              taxObject.get(MessageConstants.DOMAIN));
          taxCompetency.set(AJEntityDCACompetencyReport.TAX_STANDARD_ID,
              taxObject.get(MessageConstants.STANDARDS));
          taxCompetency.set(AJEntityDCACompetencyReport.TAX_MICRO_STANDARD_ID,
