...
public class OfflineStudentReportingHandler implements DBHandler {
  ...
  private static final Logger LOGGER = LoggerFactory
      .getLogger(OfflineStudentReportingHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (requestPayload.containsKey(AJEntityReporting.TIMESPENT)
        && requestPayload.getLong(AJEntityReporting.TIMESPENT) != null) {
      this.totalResTS = requestPayload.getLong(AJEntityReporting.TIMESPENT);
    }
    if (requestPayload.containsKey(AJEntityReporting.REACTION)
        && requestPayload.getLong(AJEntityReporting.REACTION) != null) {
      this.reaction = requestPayload.getLong(AJEntityReporting.REACTION);
    }
    if (requestPayload.containsKey(AJEntityReporting.SCORE)
        && requestPayload.getDouble(AJEntityReporting.SCORE) != null) {
      this.finalScore = requestPayload.getDouble(AJEntityReporting.SCORE);
    }
    if (requestPayload.containsKey(AJEntityReporting.MAX_SCORE)
        && requestPayload.getDouble(AJEntityReporting.MAX_SCORE) != null) {
      this.finalMaxScore = requestPayload.getDouble(AJEntityReporting.MAX_SCORE);
    }
    if (requestPayload.containsKey(AJEntityReporting.QUESTION_COUNT)
    ...
    if (collectionType.equalsIgnoreCase(EventConstants.EXTERNAL_ASSESSMENT) || collectionType
      ...
      Double percentScore = (requestPayload.getValue(PERCENT_SCORE) != null) ? Double
          .valueOf(requestPayload.getValue(PERCENT_SCORE).toString()) : null;
      if (percentScore != null) {
        ...
        this.isGraded = true;
          ...
          requestPayload.getValue(AJEntityReporting.SCORE) == null
              || requestPayload.getValue(AJEntityReporting.MAX_SCORE) == null)) {
        return new ExecutionResult<>(
            MessageResponseFactory.createInvalidRequestResponse("Invalid Json Payload"),
            ExecutionResult.ExecutionStatus.FAILED);
      } else if (requestPayload.getValue(AJEntityReporting.SCORE) != null
          && requestPayload.getValue(AJEntityReporting.MAX_SCORE) != null) {
        Double rawScore = Double
            .valueOf(requestPayload.getValue(AJEntityReporting.SCORE).toString());
        Double maxScore = Double
            .valueOf(requestPayload.getValue(AJEntityReporting.MAX_SCORE).toString());
        Double score = null;
    ...
    new DefAJEntityReportingBuilder()
        .build(baseReports, requestPayload, AJEntityReporting.getConverterRegistry());
    ...
    duplicateRow =
        AJEntityReporting.findBySQL(AJEntityReporting.CHECK_DUPLICATE_COLLECTION_EVENT, userId,
            ...
            baseReports.get(AJEntityReporting.COLLECTION_OID), EventConstants.STOP,
        ...
        if (collectionType.equalsIgnoreCase(EventConstants.EXTERNAL_COLLECTION) && userIds != null
          ...
          userIds.forEach(user -> {
            ...
            if (baseReports.insert()) {
              ...
              LOGGER.error("Error while inserting offline student event into Reports DB: " + context
                  .request().toString());
            }
          });
          ...
          LOGGER.error(
              "Error while inserting offline student event into Reports DB: " + context.request()
                  .toString());
        }
      } else {
  ...
  private ExecutionResult<MessageResponse> processResourcePlayData(JsonObject requestPayload,
    ...
    if (requestPayload.getJsonArray(RESOURCES) != null && !requestPayload.getJsonArray(RESOURCES)
        .isEmpty()) {
      ...
      for (Object res : resources) {
        ...
        duplicateRow =
            AJEntityReporting.findBySQL(AJEntityReporting.FIND_RESOURCE_EVENT, userId, collectionId,
                requestPayload.getString(AJEntityReporting.SESSION_ID),
                resource.getString(AJEntityReporting.RESOURCE_ID), EventConstants.STOP);

        baseReport
            .set(AJEntityReporting.RESOURCE_ID, resource.getString(AJEntityReporting.RESOURCE_ID));
