 ...
 * 
 */
public class OfflineStudentPerfEventRDAHandler implements DBHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(OfflineStudentPerfEventRDAHandler.class);
    ...
    public OfflineStudentPerfEventRDAHandler(RDAProcessorContext context) {
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        try {
            ...
            long reac = collectionEvent.getReaction() != 0 ? collectionEvent.getReaction() : 0;
            LazyList<AJEntityCollectionPerformance> avgReactionOfItem =
                AJEntityCollectionPerformance.findBySQL(AJEntityReporting.FETCH_AVG_REACTION_OF_COLL_BY_SESSION, collectionEvent.getSessionId(), collectionEvent.getCollectionId());
            if (avgReactionOfItem != null && !avgReactionOfItem.isEmpty()) {
                AJEntityCollectionPerformance reaction = avgReactionOfItem.get(0);
                reac = reaction != null ? reaction.getLong(AJEntityReporting.REACTION) : 0;
            }
            collectionReport.set("reaction", reac);
            ...
            if (collectionReport.isValid()) {
                if (duplicateRow == null || duplicateRow.isEmpty()) {
                    ...
                    duplicateRow.forEach(dup -> {
                        ...
                        Base.exec(AJEntityCollectionPerformance.UPDATE_COLLECTION_METRICS, view, ts, collectionEvent.getScore(), collectionEvent.getMaxScore(), react, collectionEvent.getIsGraded(), collectionEvent.getStatus(),
