...
public class DCAOfflineStudentReportingHandler implements DBHandler {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DCAOfflineStudentReportingHandler.class);
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        ...
        if (requestPayload.containsKey(AJEntityDailyClassActivity.SCORE) && requestPayload.getDouble(AJEntityDailyClassActivity.SCORE) != null) this.finalScore = requestPayload.getDouble(AJEntityDailyClassActivity.SCORE);
        if (requestPayload.containsKey(AJEntityDailyClassActivity.MAX_SCORE) && requestPayload.getDouble(AJEntityDailyClassActivity.MAX_SCORE) != null) this.finalMaxScore = requestPayload.getDouble(AJEntityDailyClassActivity.MAX_SCORE);
        if (requestPayload.containsKey(AJEntityDailyClassActivity.QUESTION_COUNT) && requestPayload.getInteger(AJEntityDailyClassActivity.QUESTION_COUNT) != null) this.questionCount = requestPayload.getInteger(AJEntityDailyClassActivity.QUESTION_COUNT);
        ...
        if (collectionType.equalsIgnoreCase(EventConstants.EXTERNAL_ASSESSMENT) || collectionType.equalsIgnoreCase(EventConstants.EXTERNAL_COLLECTION)) {
            ...
            if (percentScore != null) {
                ...
                Double rawScore = Double.valueOf(requestPayload.getValue(AJEntityDailyClassActivity.SCORE).toString());
                Double maxScore = Double.valueOf(requestPayload.getValue(AJEntityDailyClassActivity.MAX_SCORE).toString());
                Double score = null;
        ...
        new DefAJEntityDailyClassActivityBuilder().build(dcaReport, requestPayload, AJEntityDailyClassActivity.getConverterRegistry());
        
        duplicateRow =
            AJEntityDailyClassActivity.findBySQL(AJEntityDailyClassActivity.FIND_COLLECTION_EVENT,
                dcaReport.get(AJEntityDailyClassActivity.SESSION_ID), dcaReport.get(AJEntityDailyClassActivity.COLLECTION_OID), EventConstants.STOP, EventConstants.COLLECTION_PLAY);
    ...
    private ExecutionResult<MessageResponse> processResourcePlayData(JsonObject requestPayload, String collectionId, String userId, long ts) {
        if (requestPayload.getJsonArray(RESOURCES) != null && !requestPayload.getJsonArray(RESOURCES).isEmpty()) {
            ...
            for (Object res : resources) {
                ...
                duplicateRow =
                    AJEntityDailyClassActivity.findBySQL(AJEntityDailyClassActivity.FIND_RESOURCE_EVENT, collectionId, requestPayload.getString(AJEntityDailyClassActivity.SESSION_ID), resource.getString(AJEntityDailyClassActivity.RESOURCE_ID), EventConstants.STOP);
                
                dcaReport.set(AJEntityDailyClassActivity.COLLECTION_OID, collectionId);
                dcaReport.set(AJEntityDailyClassActivity.RESOURCE_ID, resource.getString(AJEntityDailyClassActivity.RESOURCE_ID));
                dcaReport.set(AJEntityDailyClassActivity.GOORUUID, userId);
                dcaReport.set(AJEntityDailyClassActivity.EVENTNAME, EventConstants.COLLECTION_RESOURCE_PLAY);
                dcaReport.set(AJEntityDailyClassActivity.EVENTTYPE, EventConstants.STOP);
                ...
                new DefAJEntityDailyClassActivityBuilder().build(dcaReport, requestPayload, AJEntityDailyClassActivity.getConverterRegistry());
                new DefAJEntityDailyClassActivityBuilder().build(dcaReport, resource, AJEntityDailyClassActivity.getConverterRegistry());
                ...
                if (dcaReport.isValid()) {
                    if (duplicateRow == null || duplicateRow.isEmpty()) {
                        if (dcaReport.insert()) {
                            LOGGER.info("Offline Student collection.resource.play event inserted successfully in Reports DB");
                            GEPEventDispatcher eventDispatcher = new GEPEventDispatcher(dcaReport, null, null, null, System.currentTimeMillis());
