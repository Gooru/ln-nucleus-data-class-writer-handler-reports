        ...
        if (duplicateRow != null && !duplicateRow.isEmpty()) {
          ...
          duplicateRow.forEach(dup -> {
            ...
            LazyList<AJEntityCollectionPerformance> avgReactionOfItem =
                AJEntityCollectionPerformance
                    .findBySQL(AJEntityReporting.FETCH_AVG_REACTION_OF_COLL_BY_SESSION,
                        event.getSessionId(), event.getCollectionId());
            ...
            Base.exec(AJEntityCollectionPerformance.UPDATE_COLLECTION_METRICS, view, ts,
                event.getScore(), event.getMaxScore(), react, event.getIsGraded(), true,
                new Timestamp(event.getActivityTime()), id);
          });
        } else {
          LOGGER.warn(
              "RDA-CSTPE:: Update Skipped! coll start event either missing or some issue. RDA-EVENT: {}",
              context.request());
        }
      }
    } catch (Exception e) {
